...
public sealed class NumberExpression : ExpressionNode {
...
public sealed class BinaryExpression(ExpressionNode left, SyntaxToken opToken, ExpressionNode right) : ExpressionNode {
    public override SyntaxKind Kind { get; } = opToken.Kind switch {
            SyntaxKind.Plus => SyntaxKind.AdditonExpresion,
            ...
            SyntaxKind.NotEqualTo => SyntaxKind.NotEqualToExression,
...
public sealed class LocalDecarationsExpression(ExpressionNode varDeclaration, ExpressionNode? localDecarationTail) : ExpressionNode
{
    public ExpressionNode VarDeclaration { get; } = varDeclaration;
    
    public ExpressionNode? LocalDecarationTail { get; } = localDecarationTail;

    public override SyntaxKind Kind => SyntaxKind.LocalDecarationsExpression;

    public override IEnumerable<ExpressionNode> GetChildren()
    {
        yield return VarDeclaration;
        if (LocalDecarationTail is not null)
            yield return LocalDecarationTail;
    }
}

public sealed class LocalDecarationsTailExpression(ExpressionNode varDeclaration, ExpressionNode? localDecarationTail) : ExpressionNode
{
    public ExpressionNode VarDeclaration { get; } = varDeclaration;

    public ExpressionNode? LocalDecarationTail { get; } = localDecarationTail;

    public override SyntaxKind Kind => SyntaxKind.LocalDecarationTailExpression;
...
public sealed class IterationStatementEpxression(ExpressionNode logicEpxr, ExpressionNode? block) : ExpressionNode
{
    public ExpressionNode LogicEpxr { get; } = logicEpxr;
    
    public ExpressionNode? Block  { get; } = block ;

    public override SyntaxKind Kind => SyntaxKind.IterationStatementEpxression;

    public override IEnumerable<ExpressionNode> GetChildren()
    {
        yield return LogicEpxr;
