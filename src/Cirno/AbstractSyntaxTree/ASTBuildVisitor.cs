...
internal sealed class ASTBuildVisitor
{
    ...
    private static void VisitCompoundStatement(in CompoundStatementExpression expr, out CompoundStatementNode node)
    {
        ...
        if (expr.LocalDeclarations is LocalDecarationsExpression localDeclExpr)
        {
            ...
            var localDeclTail = localDeclExpr.LocalDecarationTail;
            while (localDeclTail is not null)
            {
                if (localDeclTail is not LocalDecarationsTailExpression tail)
                    throw new Exception($"Unexpected type in {nameof(VisitCompoundStatement)}, expected {nameof(LocalDecarationsTailExpression)}");

                var localDecarationTail = tail.LocalDecarationTail;
                localDeclTail = tail.LocalDecarationTail;
        ...
        {
            throw new Exception($"Unexpected type in {nameof(VisitCompoundStatement)}, expected {nameof(LocalDecarationsExpression)} or null");
        }
        ...
        if (expr.StatementList is StatementListTailExpression stmtListExpr)
        {
            ...
            while (curStmtListTail is not null)
            {
                ...
                switch (stmtExpr)
                {
                    ...
                    case IterationStatementEpxression iterExpr:
                        {
                            VisitBinaryExpression(iterExpr.LogicEpxr, out var logicExprNode);
                            StatementNode stmt;
